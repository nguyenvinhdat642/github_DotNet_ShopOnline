...
namespace WebApplication1.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.sanphambanchay = db.products.OrderByDescending(s=>s.quantity_sold).Take(12).ToList(); //Lấy 12 sản phẩm làm sản phẩm bán chạy
            ViewBag.sanphammoinhat = db.products.OrderByDescending(s=>s.created).Take(12).ToList(); //Lấy 12 sản phẩm tiếp theo làm sản phẩm mới nhất
            ViewBag.giamgiathapnhat = db.products.OrderByDescending(s=>s.discount).Take(3).ToList(); //Lấy 3 sản phẩm tiếp theo làm sản phẩm giảm giá thấp nhất
        ...
        public ActionResult Product_Details(string id)
        {
            ...
            else
            {
                ...
                //Sẩn phẩm cùng loại
                ViewBag.sanphamtuongtu = db.products.Where(s=>s.catalog_id ==item.catalog_id).ToList().Take(6); //Lấy 6 sản phẩm 
                ViewBag.sanphamkhacloai = db.products.Where(s => s.catalog_id != item.catalog_id).ToList().Take(6); //Lấy 6 sản phẩm
                return View(item);
            }
